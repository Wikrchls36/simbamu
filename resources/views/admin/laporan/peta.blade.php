...
<head>
    ...
    <style>
        ...
        .btn-back-floating { position: absolute; top: 105px; left: 20px; width: 45px; height: 45px; background-color: #ffffff; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #333; text-decoration: none; box-shadow: 0 4px 15px rgba(0,0,0,0.2); z-index: 1000; transition: all 0.3s ease; font-size: 1.1rem; border: 2px solid transparent;}
        ...
        .map-legend { position: absolute; bottom: 30px; right: 20px; background: rgba(255, 255, 255, 0.95); padding: 15px 20px; border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.15); z-index: 1000; border: 1px solid #eaeaea; font-family: 'Poppins', sans-serif; backdrop-filter: blur(5px); }
        .map-legend h6 { font-weight: 700; font-size: 13px; margin-bottom: 12px; color: #333; border-bottom: 2px solid #0047ba; padding-bottom: 5px; display: inline-block; }
        .legend-item { display: flex; align-items: center; margin-bottom: 8px; font-size: 12px; font-weight: 500; color: #555; }
        .legend-icon { width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 11px; margin-right: 10px; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
        .legend-icon.banjir { background-color: #0d6efd; }
        .legend-icon.karhutla { background-color: #dc3545; }
    </style>
</head>
<body>
    ...
    <div class="map-legend">
        ...
        <div class="legend-item">
            <div class="legend-icon banjir"><i class="fas fa-water"></i></div>
            Titik Bencana Banjir
        </div>
        <div class="legend-item mb-0">
            <div class="legend-icon karhutla"><i class="fas fa-fire"></i></div>
            Titik Bencana Karhutla
        </div>
    </div>
    ...
    <script>
        ...
        const latFocus = urlParams.get('lat');
        const lngFocus = urlParams.get('lng');

        if (latFocus && lngFocus) {
            defaultCenter = [parseFloat(latFocus), parseFloat(lngFocus)];
            defaultZoom = 15;
        }

        var map = L.map('mapUser', { zoomControl: false }).setView(defaultCenter, defaultZoom);
