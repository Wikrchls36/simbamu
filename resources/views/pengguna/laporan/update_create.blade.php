...
<head>
    ...
    <style>
        ...
        .section-header { 
            ...
            color: white; padding: 12px 20px; font-weight: 600; border-radius: 10px; 
            margin-top: 45px; margin-bottom: 25px; text-transform: uppercase; 
            font-size: 0.95rem; letter-spacing: 0.5px; box-shadow: 0 4px 10px rgba(0, 71, 186, 0.15);
        }
        ...
        .input-locked { background-color: #e9ecef; cursor: not-allowed; font-weight: 600; color: #6c757d; }
            ...
            <div class="card p-4 p-md-5">
                ...
                <div class="d-flex justify-content-between align-items-start mb-4 border-bottom pb-4">
                    <div class="text-center flex-grow-1 px-3">
                        <span class="badge bg-warning text-dark rounded-pill mb-2 px-3 py-2 fw-medium"><i class="fas fa-edit me-1"></i> UPDATE SITREP BERIKUTNYA</span>
                ...
                <form action="{{ route('pengguna.laporan.store_update', $laporan->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="alert alert-info border-0 shadow-sm rounded-3 small mb-4 py-3">
                        <i class="fas fa-lightbulb me-2 fs-6"></i> <strong>Petunjuk:</strong> Data di bawah ini ditarik dari laporan (SitRep) terakhir Anda. Silakan ubah atau tambahkan informasi yang perlu diperbarui.
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <input type="hidden" name="latitude" id="pusat_lat" value="{{ $laporan->latitude }}">
                        <input type="hidden" name="longitude" id="pusat_lng" value="{{ $laporan->longitude }}">
                    </div>
                    ...
                    <div class="table-responsive mb-4">
                        <table class="table table-bordered align-middle" id="tb_waktu">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><input type="text" name="wk_waktu[]" class="form-control" value="{{ $waktu }}" placeholder="Jam / Tgl"></td>
                                    ...
                                    <td>
                                        <div class="input-group input-group-sm">
                                            ...
                                            <button class="btn btn-primary" type="button" onclick="bukaPeta(this)" title="Pilih di Peta"><i class="fas fa-map-marker-alt"></i></button>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        @if($index == 0)
                                            <button type="button" class="btn btn-add-row btn-sm w-100" onclick="addRow('tb_waktu', 'waktu')"><i class="fas fa-plus"></i></button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-sm w-100 shadow-sm" onclick="this.parentElement.parentElement.remove()"><i class="fas fa-trash-alt"></i></button>
                                        @endif
                                    </td>
                                </tr>
                                @empty
                    ...
                    <div class="row g-2 mb-4">
                        <div class="col"><label class="form-label small text-center w-100">Meninggal</label><input type="number" name="dampak_meninggal" class="form-control text-center fw-bold text-danger" value="{{ $latestSitrep->dampak_meninggal ?? 0 }}" min="0"></div>
                        <div class="col"><label class="form-label small text-center w-100">Luka-luka</label><input type="number" name="dampak_luka" class="form-control text-center fw-bold text-warning" value="{{ $latestSitrep->dampak_luka ?? 0 }}" min="0"></div>
                        <div class="col"><label class="form-label small text-center w-100">Hilang</label><input type="number" name="dampak_hilang" class="form-control text-center fw-bold text-secondary" value="{{ $latestSitrep->dampak_hilang ?? 0 }}" min="0"></div>
                        <div class="col"><label class="form-label small text-center w-100">Pengungsi</label><input type="number" name="dampak_pengungsi" class="form-control text-center fw-bold text-info" value="{{ $latestSitrep->dampak_pengungsi ?? 0 }}" min="0"></div>
                        <div class="col"><label class="form-label small text-center w-100">Terdampak</label><input type="number" name="dampak_terdampak" class="form-control text-center fw-bold text-primary" value="{{ $latestSitrep->dampak_terdampak ?? 0 }}" min="0"></div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6"><label class="form-label">Lokasi Poskor Muhammadiyah</label><textarea name="lokasi_poskor" class="form-control" rows="2">{{ $latestSitrep->lokasi_poskor ?? '' }}</textarea></div>
                        <div class="col-md-6"><label class="form-label">Lokasi Pos Pelayanan Muhammadiyah (Opsional)</label><textarea name="lokasi_pos_pelayanan" class="form-control" rows="2">{{ $latestSitrep->lokasi_pos_pelayanan ?? '' }}</textarea></div>
                    </div>
                    ...
                    <textarea name="situasi_terkini" class="form-control" rows="4" placeholder="Bagaimana kondisi terkini di lapangan saat update ini dibuat?">{{ $latestSitrep->situasi_terkini ?? '' }}</textarea>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle" id="tb_respon">
                            <thead><tr><th>Kluster</th><th>Lokasi</th><th>Penerima Manfaat / Keterangan</th><th width="50">Aksi</th></tr></thead>
                            <tbody>
                                @php 
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($index == 0)
                                            <button type="button" class="btn btn-add-row btn-sm w-100" onclick="addRow('tb_respon', 'respon')"><i class="fas fa-plus"></i></button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-sm w-100 shadow-sm" onclick="this.parentElement.parentElement.remove()"><i class="fas fa-trash-alt"></i></button>
                                        @endif
                                    </td>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle" id="tb_manfaat">
                            <thead><tr><th>Kegiatan</th><th>Tanggal Kegiatan</th><th>Jumlah Penerima Manfaat</th><th width="50">Aksi</th></tr></thead>
                            <tbody>
                                @php 
                                ...
                                <tr>
                                    ...
                                    <td><input type="date" name="pm_tanggal[]" class="form-control" value="{{ $pm_tanggal[$index] ?? '' }}"></td>
                                    <td><input type="text" name="pm_jumlah[]" class="form-control" value="{{ $pm_jumlah[$index] ?? '' }}"></td>
                                    <td class="text-center">
                                        @if($index == 0)
                                            <button type="button" class="btn btn-add-row btn-sm w-100" onclick="addRow('tb_manfaat', 'manfaat')"><i class="fas fa-plus"></i></button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-sm w-100 shadow-sm" onclick="this.parentElement.parentElement.remove()"><i class="fas fa-trash-alt"></i></button>
                                        @endif
                                    </td>
                    ...
                    <div class="table-responsive mb-3">
                        <table class="table table-bordered align-middle" id="tb_tim">
                            <thead><tr><th>Kluster</th><th>Total</th><th>Pulang</th><th>Bertugas</th><th width="50">Aksi</th></tr></thead>
                            <tbody>
                                @php 
                                ...
                                <tr>
                                    ...
                                    <td><input type="number" name="tim_total[]" class="form-control text-center" value="{{ $tim_total[$index] ?? 0 }}" min="0"></td>
                                    <td><input type="number" name="tim_pulang[]" class="form-control text-center" value="{{ $tim_pulang[$index] ?? 0 }}" min="0"></td>
                                    <td><input type="number" name="tim_bertugas[]" class="form-control text-center" value="{{ $tim_bertugas[$index] ?? 0 }}" min="0"></td>
                                    <td class="text-center">
                                        @if($index == 0)
                                            <button type="button" class="btn btn-add-row btn-sm w-100" onclick="addRow('tb_tim', 'tim')"><i class="fas fa-plus"></i></button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-sm w-100 shadow-sm" onclick="this.parentElement.parentElement.remove()"><i class="fas fa-trash-alt"></i></button>
                                        @endif
                                    </td>
                    ...
                    <div class="row g-3 mb-3 bg-light p-3 rounded-3 border">
                        <div class="col-md-4"><label class="form-label mt-0">Total</label><input type="number" name="tim_total_semua" class="form-control fw-bold text-primary" value="{{ $latestSitrep->tim_total_semua ?? 0 }}" min="0"></div>
                        <div class="col-md-4"><label class="form-label mt-0">Laki-Laki</label><input type="number" name="tim_laki" class="form-control" value="{{ $latestSitrep->tim_laki ?? 0 }}" min="0"></div>
                        <div class="col-md-4"><label class="form-label mt-0">Perempuan</label><input type="number" name="tim_perempuan" class="form-control" value="{{ $latestSitrep->tim_perempuan ?? 0 }}" min="0"></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle" id="tb_kebutuhan">
                            <thead><tr><th>Kebutuhan</th><th>Jumlah Kebutuhan</th><th width="50">Aksi</th></tr></thead>
                            <tbody>
                                @php 
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($index == 0)
                                            <button type="button" class="btn btn-add-row btn-sm w-100" onclick="addRow('tb_kebutuhan', 'kebutuhan')"><i class="fas fa-plus"></i></button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-sm w-100 shadow-sm" onclick="this.parentElement.parentElement.remove()"><i class="fas fa-trash-alt"></i></button>
                                        @endif
                                    </td>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle" id="tb_kontak">
                            <thead><tr><th>Nama </th><th>No Telepon / HP </th><th width="50">Aksi</th></tr></thead>
                            <tbody>
                                @php 
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($index == 0)
                                            <button type="button" class="btn btn-add-row btn-sm w-100" onclick="addRow('tb_kontak', 'kontak')"><i class="fas fa-plus"></i></button>
                                        @else
                                            <button type="button" class="btn btn-danger btn-sm w-100 shadow-sm" onclick="this.parentElement.parentElement.remove()"><i class="fas fa-trash-alt"></i></button>
                                        @endif
                                    </td>
                    ...
                    <div class="alert alert-secondary border p-4 mb-4" style="background-color: #f8f9fa;">
                        ...
                        <div class="col-md-12 bg-white p-4 rounded-3 border">
                            <h6 class="fw-bold text-dark mb-2"><i class="fas fa-paperclip me-2 text-primary"></i>LAMPIRAN (FOTO AKTIVITAS dan LAMPIRAN LAPORAN OPSIONAL JIKA ADA)</h6>
                            <label class="form-label text-muted d-block mb-3" style="font-size: 0.8rem;">Unggah dokumentasi aktivitas terbaru jika ada.</label>
                    ...
                    <div class="d-flex gap-3 pt-3 mt-5 border-top">
                        ...
                        <button type="submit" class="btn btn-primary btn-lg fw-bold rounded-pill shadow flex-grow-1" style="background: linear-gradient(90deg, #0047ba, #1aa4f6); border: none;">
                            <i class="fas fa-paper-plane me-2"></i> KIRIM LAPORAN TERBARU SEKARANG
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-light border-0 px-4 py-3">
                <h5 class="modal-title fw-bold text-primary" id="modalPetaLabel"><i class="fas fa-map-marked-alt me-2"></i>Ubah Titik Lokasi Bencana</h5>
                ...
                <div class="position-absolute bottom-0 start-50 translate-middle-x mb-4 w-75" style="z-index: 1000;">
                    <div class="card shadow-lg border-0 rounded-pill px-4 py-3 text-center bg-white" style="background: rgba(255,255,255,0.95) !important; backdrop-filter: blur(5px);">
                        <span class="fw-bold text-dark fs-6" id="kordinatTerpilih">
                            <i class="fas fa-hand-pointer text-primary me-2 pulse-animation"></i> Klik area pada peta untuk memindahkan pin
                        </span>
                    </div>
                </div>
            </div>
...
<script>
    // FUNGSI TAMBAH BARIS TABEL (Dengan penyesuaian styling)
...
<script>
    ...
    function bukaPeta(button) {
        ...
        document.getElementById('modalPeta').addEventListener('shown.bs.modal', function () {
            ...
            if (inputLatAktif.value && inputLngAktif.value) {
                ...
                markerPicker = null;
                document.getElementById('kordinatTerpilih').innerHTML = '<i class="fas fa-hand-pointer text-primary me-2"></i> Klik area pada peta untuk memindahkan pin';
    ...
    document.getElementById('btnSimpanKoordinat').addEventListener('click', function() {
        if (tempLat && tempLng && inputLatAktif && inputLngAktif) {
            ...
            alert('Harap klik lokasi kejadian pada peta terlebih dahulu untuk memindahkan pin!');
        }
    });
</script>
...
<script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> MENGUNGGAH UPDATE...';
