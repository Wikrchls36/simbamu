            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="d-flex justify-content-center flex-wrap gap-3 mt-4">
                        ...
                        <a href="{{ route('pengguna.peta.lihat') }}" class="btn btn-primary rounded-pill px-4 py-2 fw-bold d-inline-flex align-items-center" style="box-shadow: none !important; transition: none; transform: none;">
...
<script>
    // 1. INISIALISASI PETA PREVIEW
    // Mengaktifkan zoomControl agar muncul tombol + dan - seperti di Admin
    var map = L.map('map-preview', {
        zoomControl: true 
    }).setView([-0.2787, 111.4753], 6);
    
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; MDMC Kalbar | SIMBAMU'
    }).addTo(map);

    // 2. LOAD GEOJSON KALBAR (POLYGON BIRU)
    fetch('/data/kalbar.geojson')
        .then(res => res.json())
        .then(data => {
            L.geoJSON(data, {
                style: { 
                    color: '#0047ba', // Garis Biru MDMC
                    weight: 1.5, 
                    fillColor: '#1aa4f6', 
                    fillOpacity: 0.1 
                }
        ...
        .catch(err => console.error("GeoJSON tidak ditemukan, cek folder public/data/"));
</script>
</body>
</html>