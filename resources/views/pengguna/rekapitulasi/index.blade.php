...
<head>
    ...
    <title>Rekapitulasi Daerah - SIMBAMU</title>
...
<div class="d-flex">
    <nav id="sidebar">
        ...
        <ul class="nav mt-3">
            ...
            <li class="nav-item"><a href="/pengguna/rekapitulasi" class="nav-link"><i class="fas fa-chart-area fa-fw me-3"></i> Rekap Laporan</a></li>
        </ul>
    </nav>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm px-4">
            ...
            <div class="ms-auto dropdown">
                <a href="#" class="d-flex align-items-center text-decoration-none text-dark profile-toggle-btn" data-bs-toggle="dropdown">
                    <img src="{{ Auth::user()->profile_photo ? asset('storage/' . Auth::user()->profile_photo) : asset('images/logo-mdmc.png') }}" class="profile-img me-2 shadow-sm" style="object-fit: cover;" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=0047ba&color=fff'">
        ...
        <div class="container-fluid p-4 p-md-5 flex-grow-1">
            ...
            <div class="row align-items-center mb-4 pb-2 border-bottom judul-halaman">
                <div class="col-lg-5 mb-3 mb-lg-0">
                    <h3 class="fw-bold text-dark m-0">Rekapitulasi Laporan Daerah</h3>
                    <p class="text-muted m-0 mt-1" style="font-size: 14px;">
                        Anda memiliki total <strong>{{ $totalKejadian }} laporan kejadian</strong> bencana
                    </p>
                </div>
                <div class="col-lg-7">
                    <form method="GET" action="{{ route('pengguna.rekapitulasi.index') }}" class="d-flex gap-2 justify-content-lg-end flex-wrap filter-section">
                        ...
                        <select name="bulan" class="form-select filter-select" style="width: auto; min-width: 130px;" onchange="this.form.submit()">
                            ...
                            @foreach($namaBulanList as $index => $namaBulan)
                                <option value="{{ sprintf('%02d', $index + 1) }}" {{ $filterBulan == sprintf('%02d', $index + 1) ? 'selected' : '' }}>{{ $namaBulan }}</option>
                            @endforeach
                        </select>
                        <a href="{{ route('pengguna.rekapitulasi.cetak', request()->query()) }}" target="_blank" class="btn btn-dark fw-bold rounded-3 px-3 shadow-sm d-flex align-items-center" style="font-size: 14px;" title="Cetak Laporan Formal">
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-12">
                    <div class="rekap-card p-4 shadow-sm bg-success text-white border-0" style="background: linear-gradient(135deg, #198754, #20c997);">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="fw-bold text-uppercase mb-1 text-white-50" style="font-size: 13px; letter-spacing: 1px;">Total Relawan Yang Dikerahkan</h6>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white border-bottom-0 pt-4 px-4 pb-3 d-flex align-items-center">
                    ...
                    <h5 class="fw-bold text-dark m-0">Rincian Histori Laporan Anda</h5>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        ...
                        <thead>
                            <tr>
                                <th class="text-start ps-4">No</th>
                                <th>Tanggal</th>
                                <th class="text-primary">BENCANA</th>
                                <th>STATUS</th>
                                <th>MENINGGAL</th>
                                <th>LUKA-LUKA</th>
                                <th>TERDAMPAK</th>
                        ...
                        <tbody>
                            ...
                            @php $sitrep = $lap->updates->first(); @endphp
                            <tr>
                                <td class="text-start ps-4 fw-bold text-dark">{{ $index + 1 }}</td>
                                <td class="fw-medium">{{ \Carbon\Carbon::parse($lap->created_at)->format('d/m/Y') }}</td>
                                
                                <!-- Kolom Fokus Bencana (Biru Bold) -->
                                <td class="fw-bold text-primary bg-primary bg-opacity-10">{{ $lap->jenis_bencana }}</td>
                                
                                <td>
                                    <span class="badge {{ $lap->status == 'Aktif' ? 'bg-primary' : 'bg-success' }} px-3 py-2 rounded-pill shadow-sm" style="font-weight: 500;">
                                        {{ $lap->status }}
                                    </span>
                                </td>
                                
                                <td>{{ number_format($sitrep->dampak_meninggal ?? 0, 0, ',', '.') }}</td>
                                <td>{{ number_format($sitrep->dampak_luka ?? 0, 0, ',', '.') }}</td>
                                <td class="fw-bold text-primary">{{ number_format(($sitrep->dampak_terdampak ?? 0) + ($sitrep->dampak_pengungsi ?? 0), 0, ',', '.') }}</td>
                                <td class="pe-4 fw-bold text-success">{{ number_format($sitrep->tim_total_semua ?? 0, 0, ',', '.') }}</td>
                            </tr>
                            @empty
                            <tr>
        ...
        <footer class="text-center py-4 mt-auto filter-section">
            <small class="text-muted">
                <i class="far fa-copyright"></i> MDMC KALIMANTAN BARAT {{ date('Y') }} - SOLID BERGERAK MONITOR | DIKELOLA OLEH BIDANG TANGGAP DARURAT
            </small>
        </footer>
    </div>
</div>
